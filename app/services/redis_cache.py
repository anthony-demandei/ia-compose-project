...
class RedisCache:
    ...
    def _init_connection(self):
        ...
        try:
            ...
            ] = self.settings.redis_connection_timeout
            ...
            logger.warning(
                f"⚠️ Redis connection failed, using in-memory cache: {str(e)}"
            )
            self._connected = False
        ...
        if await self._ensure_connection():
            try:
                ...
                if cached_data:
                    ...
                    logger.info(
                        f"✅ Questions retrieved from Redis cache (key: {cache_key[:20]}...)"
                    )
                    return questions
            except Exception as e:
        ...
        if cache_key in self._memory_cache:
            ...
            if cache_entry.get("expires_at", 0) > 0:  # Check if not expired
                ...
                if time.time() < cache_entry["expires_at"]:
                    ...
                    questions = [Question(**q) for q in questions_data]
                    logger.info(
                        f"✅ Questions retrieved from memory cache (key: {cache_key[:20]}...)"
                    )
                    return questions
    ...
    async def cache_questions(
        ...
        if await self._ensure_connection():
            try:
                await self._redis_client.setex(
                    cache_key, ttl, json.dumps(questions_data)
                )
                logger.info(
                    f"✅ Questions cached in Redis (key: {cache_key[:20]}..., TTL: {ttl}s)"
                )
                return True
            except Exception as e:
        ...
        logger.info(
            f"✅ Questions cached in memory (key: {cache_key[:20]}..., TTL: {ttl}s)"
        )
        return True
    ...
    async def get_cached_document(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if await self._ensure_connection():
            try:
                ...
                if cached_data:
                    ...
                    logger.info(
                        f"✅ Document retrieved from Redis cache (session: {session_id[:8]}...)"
                    )
        ...
        if cache_key in self._memory_cache:
            ...
            if cache_entry.get("expires_at", 0) > 0:
                ...
                if time.time() < cache_entry["expires_at"]:
                    logger.info(
                        f"✅ Document retrieved from memory cache (session: {session_id[:8]}...)"
                    )
    ...
    async def cache_document(
        ...
        if await self._ensure_connection():
            try:
                await self._redis_client.setex(
                    cache_key, ttl, json.dumps(document_data)
                )
                logger.info(
                    f"✅ Document cached in Redis (session: {session_id[:8]}..., TTL: {ttl}s / {ttl/3600:.1f}h)"
                )
                return True
            except Exception as e:
        ...
        logger.info(
            f"✅ Document cached in memory (session: {session_id[:8]}..., TTL: {ttl}s / {ttl/3600:.1f}h)"
        )
        return True
    ...
    async def invalidate_document(self, session_id: str) -> bool:
        ...
        if await self._ensure_connection():
            try:
                ...
                logger.info(
                    f"✅ Document cache invalidated in Redis (session: {session_id[:8]}...)"
                )
            except Exception as e:
        ...
        if cache_key in self._memory_cache:
            ...
            logger.info(
                f"✅ Document cache invalidated in memory (session: {session_id[:8]}...)"
            )
